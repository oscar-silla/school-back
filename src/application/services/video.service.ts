...
import { VideoRepositoryPort } from "../ports/out/repository/video.repository.port";
...
export class VideoService implements VideoServicePort {
  ...
  async getVideoByRef(ref: string): Promise<Video> {
    const video: Video | null = await this.videoRepositoryAdapter.getOneByRef(
      ref
    );
  ...
  async getVideoById(id: string): Promise<Video> {
    const video: Video | null = await this.videoRepositoryAdapter.getOneById(
      id
    );
    this.throwExceptionIfNotFoundVideo(video);
    return video!;
  ...
  private throwExceptionIfNotFoundVideo(video: Video | null): void {
    if (!video) {
